...
class Bilibili():
    ...
    def __init__(self, oid):
        ...
        # 视频url
        self.video = 'https://m.bilibili.com/video/av' + str(oid) + '.html'
        print(self.video)
        html = requests.get(url=self.video, headers=headers).content.decode('utf-8')
        # 获取弹幕url的参数
        cid = re.findall(r"comment: '//comment.bilibili.com/' \+ (.*?) \+ '.xml',", html)
        print(cid)
        self.url = 'https://api.bilibili.com/x/v1/dm/list.so?oid=' + cid[0]
        print(self.url)
    ...
    def make_wordCould(self):
        ...
        if results:
            ...
            words = ''.join(words)
            # luo = np.array(Image.open('cloud.jpg'))
            w = wc(font_path='‪C:/Windows/Fonts/SIMYOU.TTF', background_color='white', width=1600, height=1600,
                   max_words=2000)
            w.generate(words)
            w.to_file('cloud.jpg')
